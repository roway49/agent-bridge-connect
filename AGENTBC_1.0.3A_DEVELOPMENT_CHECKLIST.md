...
| `SESSION-103-002` | P1，开发完成 / 真实 RC 待验收 | E2E canary teardown 只结束进程和删除临时目录，没有关闭测试创建的官方 Executor 会话 | `WT3X-001` 的 raw Codex app-server canary 调用 `thread/start` 后仅 `terminate()` 并删除 `/tmp`，相关 Codex 对话继续保留 | 为真实 E2E helper 建立 receipt 驱动的 `try/finally` teardown；成功、拒绝、超时和进程异常均精确清理本次创建的会话 |
| `SESSION-103-003` | P1，开发完成 / 真实 RC 待验收 | AgentBC 只跟踪主 Executor 的单一 `agentbc.session`，由 Executor 派生的子 Executor 对话不在 cleanup ledger | `WT3X-001` 的 Claude 主会话已按 `retain=false` 清理，但其派生的 Codex threads 没有被登记或清理 | 增加 task/run scoped auxiliary session receipt 与 cleanup 状态；任务收尾覆盖主会话及全部已登记派生会话，禁止扫描私有会话库猜测 ID |
...
| `PERM-104-002` | 延期至 1.0.4A | compatibility full 已获批后，外层 OS sandbox 仍可能拒绝同一动作；当前流程无法识别“不可由 Executor 授权改变”的稳定阻塞，反复弹窗并启动 continuation | `HTE7-001` 对 linked-worktree 共用 `.git` 的提交动作产生 13 个 run、12 次 resume；Executor 记录 9 次 full grant 后相同写入仍被外层 allow-list 拒绝 | 为阻塞建立稳定 fingerprint 与 escalation-domain 分类；同一动作获批后若仍命中相同外层 containment，立即以结构化 blocked 终止，不再申请 grant、启动 worker 或要求用户重复批准 |
| `FLOW-104-002` | 延期至 1.0.4A | 终态通知依赖报告/record-budget 成功；记录超限可导致任务已经 failed 且报告存在，但没有完成/失败弹窗和 `notification_delivery` | `HTE7-001` 先因 `completion_marker_steps_incomplete` 进入 failed，随后报告处理触发 `Task record exceeds 10240 bytes: 14183`，通知调用未执行且异常被吞掉 | 将终态状态、报告压缩、通知投递与 cleanup 资格拆成可恢复阶段；通知必须独立 best-effort 投递并持久化 receipt，input/run history 有界，报告超限不得阻断终态用户反馈 |

### 1.4 当前开发进度（2026-08-26）
...
#### 8.1.1 Update 双机最终 RC（2026-08-25）

- 验证基线为 `private/integration@4ef01ca`。为避免 2026-08-22 的旧测试包继续验证过时代码，
  本轮从同一 integration 源码构造测试专用 `1.0.2a1` seed、`1.0.3a1` clean target 和
  `1.0.3a1` Runner-start fault wheel；只改测试包版本与故障注入，不作为正式发布资产；
- Update 定向单元测试 69 项、全量 unittest 1343 项、Ruff、compileall 与 `git diff --check` 通过。
  实跑前另修正 RC 驱动的临时 TLS 证书：补齐 CA
  `basicConstraints/keyUsage` 和服务端 `SKI/AKI/serverAuth`，使 Python 3.14 严格验证能够使用
  `SSL_CERT_FILE` 信任测试 CA；该改动不改变产品下载或证书校验逻辑；
- Mac mini 与 Intel MacBook 的 fresh isolated success 场景均为 `actual=updated`、exit `0`：
  CLI 链接从旧 venv 前进到新版本目录，新 Runner 均为 `ready/match`，Codex、Claude、Hermes
  三套受管 Skill 字节全部刷新；配置与排除 cutover stamp 后的 workspace/board 数据哈希保持稳定；
- 两机 fresh isolated `runner_start` 故障场景均返回产品预期的 update error，驱动合同为 accepted：
  CLI 精确恢复旧链接、三平台 Skill 字节逐项恢复、旧 Runner `ready/match`、没有第二 Runner，
  配置、workspace data 与 board data 哈希全部一致；
- MacBook 测试资产 SHA-256：seed wheel
  `cd01c04d211e66a69289b50871172b284886a897dce967219f20a3be4501c9ce`，clean target
  `2e7e2f3768b23e6c5f305807317edb95b3014e9606e1bba58dcae5abf32d731b`，Runner fault
  `9776b2266d1cd1ff54faccbfaa9be2c2517ff875410c198c955f38c8f226901f`；机器证据位于
  `/private/tmp/agentbc-rc-3KpzZk/evidence/update-final-{mini,macbook}-{success,runner-rollback}-20260825.json`；
- MacBook 的 fresh E2E 随机根与 Runner 均自动 teardown；另行停止并删除旧的
  `/tmp/agentbc-update-manual` 隔离 Runner（PID `7779`）及本轮远端测试资产。正式
  `~/.local/bin/agentbc` 和三平台 Skill 保持 `1.0.3a1/current`，正式 Runner 已恢复为
  `ready/match`（PID `14131`）。据此 `UPD-103-001` 的双机功能 RC Gate accepted。
...
- Mac mini 临时 HTTPS feed 的 curl 与 Python 双探测、CA/server 扩展校验均通过。预检在任何
  Tap/install 之前因 `brew doctor` 非零与当时 Formula 固定的 `python@3.13` 依赖停止；证据为
...
#### 8.2 Python 依赖合同纠正（2026-08-24）

- `python@3.13` 是 `26ccbb4` 初始 Formula 中没有兼容性依据的打包硬编码，不是 AgentBC 的运行
  合同；项目最低版本始终为 `pyproject.toml` 的 `requires-python >=3.10`；
- `4d21d31` 将 Formula 改为 `depends_on "python"`，并增加不得生成 `depends_on "python@..."`
  的回归断言。Homebrew 实际解释器版本只进入 RC 证据，不形成第二套产品兼容合同；
- 系统 Python 3.9 与 Homebrew Python 3.14 分别通过 13 项 Homebrew 定向测试；88 项联合
  Update/Homebrew 测试、两套重生成 Formula 的 Ruby 语法、Ruff 与 diff check 通过；
- `/private/tmp/agentbc-homebrew-preflight-20260824.json` 证明 `dependency:python=0`、HTTPS 双探测
...
#### 8.2 Mac mini 非 service RC 验收（2026-08-24）

- 经用户明确授权，仅将 Homebrew `python@3.14` 从 `3.14.3_1` 升级到 current `3.14.7`；执行时
  禁用 cleanup/autoremove，未升级其他 Formula，原 local-alpha Runner 全程保持 ready 且 PID
  `11253` 未变化。升级后的只读预检为 `preflight_ready`、blocker `0`，7 个必需写路径全部通过；
- 首次真实 RC 在 install、`brew test` 和 `1.0.2a1 → 1.0.3a1` upgrade 后，精确暴露出
  Homebrew-owned `agentbc update` 仍先查询 release index 的实现缺陷。`40ce0ec` 将 Homebrew 所有权
  路由前移到远端检查之前，并以 Cellar 内 runtime Python 作为 argv/opt link 之外的归属证据；该
  路径不调用 remote checker、不询问、不写 cutover stamp，只返回 `brew upgrade agentbc`；
- 修复后系统 Python 下 92 项联合 Update/Homebrew 测试、Homebrew Python 3.14 下 31 项相关测试、
  Ruff、compile 与 `git diff --check` 通过。干净 detached `40ce0ec` 构建的候选包通过 build、Twine、
  manifest/hash、Ruby 语法与隔离资产校验；候选 commit 为
  `40ce0ecdee87602d7d90ec5334c4aa4c580024fb`，wheel SHA-256 为
  `4ef5573b0d214524f3d03db3e76a7177cc78144d20bb872412827b5e64fa3e73`，sdist SHA-256 为
  `50d32f10d417293445ce3b0086c43c05551492731ad9283c74f612812fe194ac`，Formula SHA-256 为
  `7c4eed6910e12039b84903e98d0c7f510b7b17b93b183e22e7e81466189c29da`；
- `/private/tmp/agentbc-homebrew-rc-execute-fix-20260824.json` 状态为 `accepted`：Mac mini 非 service
  流程完成 clean install、`brew test`、版本/help、upgrade、Homebrew update guidance、uninstall、
  untrust 与 untap；执行前后 Brew 状态、PATH 和用户配置/record/report/artifact/RunLease 哈希均
  精确一致，无 AgentBC Formula、RC Tap、trust 或 service 残留；
- `PKG-103-001` 目前仅接受 ARM64 非 service 子门禁。Intel MacBook 的 install/upgrade/uninstall、
  `brew services start/stop`、Cellar Runner identity 与最终恢复仍是必需门禁；Update 的真实成功升级
  和故障包恢复也须独立完成，因此 Phase 5 与 `SESSION-103-002/003` 仍不提前标记完成或解冻。

#### 8.2 Homebrew bottle 双架构 RC（2026-08-25）

- Intel MacBook 的 source-only Formula 失败已确认不是 AgentBC 的 Xcode/CLT 依赖：Homebrew 在
  `versions.bottle=false` 时进入 `fatal_build_from_source_checks`，因此 macOS 26.6.2 上的 Xcode 15.4 / CLT
  15.3 会在 Formula 安装逻辑前被拒绝；同机可正常安装有 bottle 的其他 Formula；
- 私有 RC 已生成并审计两个 `all` bottle：`1.0.2a1` SHA-256
  `2b57aad1acb4021a6fe900e72dbd8dec6e75eb31f56986d7123b4fd320b14db0`，`1.0.3a1` SHA-256
  `0707acd9f2a13bc5f574f8acf6552961999e0b0caca34f06ecc08b38bf23097d`。两者均无 Mach-O/ELF、绝对
  symlink 或未占位的安装前缀，Python 入口由 Homebrew 在 pour 时重定位；
- Apple Silicon 强制 bottle pour、`brew test`、version/help 与 receipt 验证通过。Intel MacBook 在不更新
  Xcode/CLT 的条件下完成 `1.0.2a1` bottle install、`1.0.2a1 -> 1.0.3a1` bottle upgrade、version/help、
  Homebrew-owned update guidance 以及 `brew services start/stop`；两版 receipt 均为
  `built_as_bottle=true`、`poured_from_bottle=true`，service 日志包含 `runner_ready`；
- Intel 上单独运行 `brew test` 仍会由 Homebrew test harness 初始化 build environment 并触发宿主 Xcode/CLT
  门禁。该命令保留为工具链合格的打包 CI/Mac mini 验收，不再作为终端用户安装或运行前置；用户侧改用
  bottle receipt、CLI version/help 和 service smoke；
- 双机结束后 AgentBC Formula、opt link、RC Tap、Formula trust 与 Brew service 均已移除，原
  `~/.local/bin/agentbc 1.0.3a1` 保持不变。详细临时证据位于
  `/private/tmp/agentbc-rc-3KpzZk/evidence/HOMEBREW_BOTTLE_RC_20260825.md`；当前 bottle 仍为私有 RC
  资产，正式发布前必须把可复现生成、纯 Python payload 审计、Formula bottle stanza 与 hash 校验纳入
  release workflow，不得直接把临时资产当正式产物。

#### 8.2.1 Homebrew 与 `1.0.3a2` 发布/测试顺序（2026-08-25）

发布分为两个彼此独立的节点，禁止把 Homebrew bootstrap 与产品正式发布混成一次不可回退操作：
...
- `UPD-103-001` 真实新旧包升级/故障注入和 `PKG-103-001` 双架构
  install/upgrade/uninstall/services/PATH/迁移 Gate 已于 2026-08-25 通过，本节解除前置冻结并成为
  下一开发项；两项 session P1 通过后才恢复会创建真实 Executor 对话的权限 E2E 与最终发布 Gate；
...
开发状态（2026-08-26）：`agent/claude@58e5986` 已实现上述两项合同，
`private/integration@0e56bb3` 完成合入。联合定向测试 227 项、隔离 HOME 下全量 unittest 1384 项、
Ruff、compileall、`git diff --check`、sdist/wheel build 与 Twine check 全部通过。当前只关闭“代码与
自动化开发”门禁；真实 Codex/Claude/Hermes 会话的官方删除 receipt、Doctor blocker 清零和零新增
遗留对话必须使用 `1.0.3a2` 候选包复验后，才能关闭 Phase 5.5 RC Gate。
...
### 8.6 审批通知机械判定与非升级型阻塞收敛（延期到 `1.0.4A / PERM-104-001～002`）
...
`PERM-104-002` 补充 compatibility grant 无法改变外层 sandbox 的边界。`HTE7-001` 已证明：
AgentBC/Executor 接受 full grant 不等于宿主 OS allow-list 已扩大，linked-worktree 的共用 `.git`
仍可能在 grant 后以完全相同的 containment 原因失败。1.0.4A 必须：

- 为受阻动作记录脱敏稳定 fingerprint、阻塞来源域（Executor policy、AgentBC policy、Runner
  PathPlan、宿主 OS containment）和一次审批后的执行结果；禁止保存 raw argv、私有路径或 token；
- 同一 task/run/session/action 在获批后再次命中相同 fingerprint 与不可升级来源域时，直接输出
  结构化 blocked 终态，不再创建新的 permission input、grant、worker 或 continuation；
- 只有可信 transport 证明阻塞来源发生变化，或用户明确请求不同动作，才能生成新的 request；
  “换措辞再申请”“普通 message/choice 要求修复 sandbox”均不能重置该单调状态；
- 回归覆盖 linked-worktree 共用 Git store、Runner allow-root、宿主 sandbox、真正可升级的 Executor
  拒绝和不同动作，要求不可升级阻塞最多一次批准、可升级动作仍按原 request 精确执行。

### 8.7 终态通知与记录预算解耦（延期到 `1.0.4A / FLOW-104-002`）

- `HTE7-001` 的任务状态和报告先写成 failed，随后 compact record 仍为 14183 bytes，超过 10240
  bytes 上限；当前异常路径在 `_write_terminal_report()` 抛错后跳过 `_notify_terminal()`，并由外层
  `except ABCError` 吞掉，因此没有 `notification_delivery`，用户看不到完成/失败弹窗；
- 终态状态落盘、报告生成/压缩、文件通知、macOS 弹窗、cleanup request 必须形成相互独立、可重放的
  阶段 receipt。报告或索引失败不得阻止终态通知；通知失败也不得回写或伪造任务业务状态；
- `agentbc.input_history`、run intervals、错误详情和通知详情必须在进入 task record 前有明确条数/字节
  上限；超限时保留首条、末条、计数和稳定摘要，完整诊断进入有界事件文件而不是无限扩张 task.json；
- terminal notifier 至少执行一次 best-effort 文件投递和一次 UI 投递，并记录成功、失败或待重试 receipt；
  Runner 重启可重放缺失投递，但不得重复弹出已经确认投递的同一 terminal event；
- 回归覆盖 callback invalid、报告不可写、record 超限、事件文件超限、DialogNotifier 失败、Runner
  崩溃/重启和并发终态，要求 status/report/notification/cleanup 最终一致且不会静默结束。
...
6. **Phase 5：交互式 update 与 Homebrew**——Update 修复已合入 `f7cbfbb`，Homebrew RC 驱动、
   Python 合同、Doctor 分类及 Homebrew-owned update 前置路由已收口到 `40ce0ec`；
...
7. **Phase 5.5：E2E/session P1 补完**——`1.0.3a1` Homebrew bootstrap 已完成；
