...
> 制定日期：2026-08-26
> 状态：**已截止并正式发布（2026-09-17）**。本清单自发布截止点起只保留为内部开发与验收档案；未完成的 P2/GUI/体验优化及已明确延期项转入后续版本，不再回写 `1.0.4A` 发布基线。
> 目标版本：AgentBC `1.0.4A` / Python `1.0.4a1`

## 发布截止记录（2026-09-17）

- 正式发布标签：`v1.0.4A`；不可变发布源提交：`bf70b1086d1b41dd35da947076d7a90e8487edef`。
- GitHub Release：`https://github.com/roway49/agent-bridge-connect/releases/tag/v1.0.4A`；PyPI：`agentbc==1.0.4a1`。
- Trusted Publishing workflow `35237661933` 成功；本地全新环境安装验证确认 CLI 与 import 版本均为 `1.0.4a1`。
- 公开 `main` 的 Homebrew Formula 收口提交为 `4e645fd1e0c765b83ed918d3b6320ded6a11b728`，指向 `v1.0.4A` sdist，SHA-256 为 `6a295a6c9d31a028bba84a5db209d92224b5cf1a4f4d38906f3a1e1442b23e19`。
- GitHub Support 工单 `#4767731` 已完成引用检查、垃圾回收与缓存视图清理并关闭；被清退提交 `4c8d7f822620d6036fa994b98732717701bc9d79` 的公开链接已验证返回 404。该提交仅保留在私有开发历史中，不属于公开发布图。
- 正式发布边界已重新核对：私有开发清单、内部手册、测试证据与临时开发材料均不在公开发布提交中。
- 截止结论：`1.0.4A` 开发清单冻结；后续问题仅按回归缺陷或下一版本需求立项，不修改已发布 tag、PyPI 文件或发布源提交。
