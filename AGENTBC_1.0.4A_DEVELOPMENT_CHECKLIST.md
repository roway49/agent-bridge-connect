...
| `FLOW-104-003-R1` | **P1 / 实现完成，待真机复验** | `E52M-003` 固定了返回码 `0`、无 callback、无可识别耗尽回执的历史反例；`JWCH-001` 进一步证明同 session continuation 可在仍有 pending step 时正常结束并误落为笼统 `completion_marker_missing` | Hermes direct/Runner/ACP 现先绑定 Task、run、官方 session、冻结资源快照和最终响应边界，再机械区分 completed、max-turn/context exhaustion、user stop、transport failure、output truncation、incomplete normal exit 与未知协议原因；只有唯一合法 callback 可完成，`incomplete_normal_exit` 保留 failed 证据并开放受审计 retry/handoff | 2032 tests passed、17 skipped；Ruff、compileall、`git diff --check` 通过；待新包真机 canary 后关闭 |
...
| 9 月 19 日—9 月 22 日 | P1：`FLOW-103-001`；`RESOURCE-104-001-R1` 已通过 | `MHZH-001` 已证明资源耗尽 Desktop 弹窗、同 session continuation 和单调 progress；`FLOW-104-001` 已移至 1.0.5A | 收口 Hermes terminal receipt 与 `FLOW-103-001` 剩余回归 |
