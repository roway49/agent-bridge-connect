"""Read-only collectors for the AgentBC doctor diagnostics.

ARCH-104-001 Slice C: this module is the single read-only owner of doctor
data collection -- config, package identity, Runner, storage, skills,
executors, permission runtime, session cleanup, and blockers.  Every
collector here performs zero writes, zero setup, zero repair, never starts
a Runner or TaskService mutation, and never touches the network.  The
stable public contract (ordering, aggregation, overall status, JSON/text
rendering) stays in ``doctor.py``, which imports these collectors directly
from this module; the bindings in ``doctor.py`` double as compatibility
aliases for every name that was previously defined there.
"""
...
# Shared collector constants (single source of truth; re-exported by doctor.py).
